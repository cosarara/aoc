import sys

result = lambda n: (n[(n.index(0)+1000)%(len(n))] +
                    n[(n.index(0)+2000)%(len(n))] +
                    n[(n.index(0)+3000)%(len(n))])
result2 = lambda m0: (look_right(m0, 1000)['v']+
                      look_right(m0, 2000)['v']+
                      look_right(m0, 3000)['v'])

VERBOSE = '-v' in sys.argv
def mix(l, verbose=VERBOSE):
    newl = list(l)
    for i in l:
        if verbose:
            print(newl, i)
        pos = newl.index(i)
        newl.pop(pos)
        newpos = (pos+i-1)%(len(newl))
        if verbose:
            print(f'moving {i} between {newl[newpos]} {newl[(newpos+1)%len(newl)]}')
        newl.insert((newpos+1)%len(newl), i)
    if verbose:
        print(newl)
    return newl

def circle(l):
    nodes = [{'v': l[0], 'next': None, 'prev': None}]
    node_map = {0: nodes[0]}
    for i, v in enumerate(l[1:]):
        prev = nodes[-1]
        n = {'v': v, "prev": prev, "next": None}
        prev['next'] = n
        nodes.append(n)
        node_map[i+1] = n
    nodes[0]['prev'] = nodes[-1]
    nodes[-1]['next'] = nodes[0]
    return node_map

def look_left(node, i):
    tmp = node
    for i in range(i):
        tmp = tmp['prev']
    return tmp

def look_right(node, i):
    tmp = node
    for i in range(i):
        tmp = tmp['next']
    return tmp

def print_circle(node_map):
    tmp = node_map[0]
    for _ in range(len(node_map)+1):
        print(tmp['v'], end=' ')
        tmp = tmp['next']
    print()

def circle_to_0list(node_map):
    l = []
    tmp = node_map[0]
    for _ in range(len(node_map)):
        l.append(tmp['v'])
        tmp = tmp['next']
    return to_0list(l)

def to_0list(l):
    i = l.index(0)
    return l[i:]+l[:i]

#def mix2(l, verbose=VERBOSE):
#    node_map = circle(l)
#    for i in l:
#        if verbose:
#            # print
#            print_circle(node_map)
#            # end print
#            print('moving', i)
#        n = node_map[i]
#        old_prev = n['prev']
#        old_next = n['next']
#        if i == 0:
#            continue
#        if i >= 0:
#            # will go after the found element
#            target_left = look_right(n, i)
#            target_right = target_left['next']
#        else:
#            # will go before the found element
#            target_right = look_left(n, -i)
#            target_left = target_right['prev']
#        if old_prev == target_left:
#            # we didn't move
#            print("didn't move", target_right['v'], target_left['v'])
#            continue
#        if verbose:
#            print('between', target_left['v'], 'and', target_right['v'])
#        # move to target
#        target_left['next'] = n
#        target_right['prev'] = n
#        n['prev'] = target_left
#        n['next'] = target_right
#        # close the gap
#        old_prev['next'] = old_next
#        old_next['prev'] = old_prev
#    if verbose:
#        print_circle(node_map)
#    return node_map

def mix3(l, verbose=VERBOSE):
    newl = list(l)
    for i in l:
        if verbose:
            print(newl, i)
        if i == 0:
            continue
        pos = newl.index(i)
        if i >= 0:
            for j in range(i):
                # swap node with the element on the right
                other = (pos+1) % len(l)
                newl[pos], newl[other] = newl[other], newl[pos]
                pos = other
        else:
            for j in range(-i):
                # swap node with element on the left
                other = (pos-1) % len(l)
                newl[pos], newl[other] = newl[other], newl[pos]
                pos = other
    if verbose:
        print(newl)
    return newl

def mix4(l, verbose=VERBOSE):
    node_map = circle(l)
    for i, v in enumerate(l):
        if verbose:
            # print
            print_circle(node_map)
            # end print
            print('moving', v)
        n = node_map[i]
        old_prev = n['prev']
        old_next = n['next']
        if v == 0:
            continue
        # close the gap
        old_prev['next'] = old_next
        old_next['prev'] = old_prev
        if v >= 0:
            # will go after the found element
            target_left = look_right(n, v)
            target_right = target_left['next']
        else:
            # will go before the found element
            target_right = look_left(n, -v)
            target_left = target_right['prev']
        #if old_prev == target_left:
            # we didn't move
            #print("didn't move", i, target_right['v'], target_left['v'])
            #continue
        #print('between', target_left['v'], 'and', target_right['v'])
        # move to target
        target_left['next'] = n
        target_right['prev'] = n
        n['prev'] = target_left
        n['next'] = target_right
    if verbose:
        print_circle(node_map)
    return node_map

import random
def randinput():
    return random.sample(range(-500, 500), random.randint(5, 200))

#for i in range(1000):
#    l = list(randinput())
#    if not 0 in l:
#        l += [0]
#        random.shuffle(l)
#    print(l)
#    out1 = to_0list(mix(l))
#    out3 = to_0list(mix3(l))
#    out4 = circle_to_0list(mix4(l))
#    print('outs', out1[:10], out3[:10], out4[:10])
#    if out1 != out3:
#        print("1 and 3 differ")
#        print(out1, out3)
#        break
#    if out1 != out4:
#        print("1 and 4 differ")
#        print(out1, out3)
#        break
#    if out3 != out4:
#        print("3 and 4 differ")
#        print(out1, out3)
#        break

example = """1
2
-3
3
-2
0
4"""

le = [int(a) for a in example.split()]
#
#le2 = [4, 0, -1]
#le3 = [-5, 0, -1, 2]
#
inp = sys.stdin.read()

l = [int(a) for a in inp.split()]

#ll = l[:1250] has issues with result2
#ll = l[0:100]+[0]

print('approach #1 - find new position by index and math')
m = mix(le)
assert result(m) == 3
m = mix(l)
print(result(m))
#m = mix(ll)
#print(result(m))

#m = mix(le2, True)
#m = mix(le3, True)

    
#print('approach #2 - circle list, closing gap at the end (wrong)')
#m = mix2(le)
#assert result2(m) == 3
#m = mix2(l)
#print(result2(m))
#
#m = mix2(le2, True)
#m = mix2(le3, True)

print('approach #3 - swapping')
m = mix3(le)
assert result(m) == 3
m = mix3(l)
print(result(m))
#m = mix3(ll)
#print(result(m))

#m = mix3(le2, True)
#m = mix3(le3, True)
#
print('approach #4 - circle list, closing gap at the start')
m = mix4(le)
##print_circle(m)
##print(result2(m))
assert result2(m[le.index(0)]) == 3
m = mix4(l)
print(result2(m[l.index(0)]), result(circle_to_0list(m)))
#m = mix4(ll)
#print(result2(m), result(circle_to_0list(m)))
#
#m = mix4(le2, True)
#m = mix4(le3, True)
